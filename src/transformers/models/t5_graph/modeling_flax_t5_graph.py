...
class FlaxT5GraphAttention(nn.Module):
    ...
    def __call__(
        ...
        if position_bias is None:
            # compute position bias (only for first layer)
            position_bias = self._create_position_bias(
                receivers, senders, key_states, query_states, attention_mask, init_cache, seq_length, causal_attention_mask_shift
            )
...
class FlaxT5GraphLayerCrossAttention(nn.Module):
    ...
    def __call__(
        ...
        print(f"shape of adj_list in encdec attention: {receivers.shape}")
        normed_hidden_states = self.layer_norm(hidden_states)
...
class FlaxT5Block(nn.Module):
    ...
    def __call__(
        ...
        do_cross_attention = self.causal and encoder_hidden_states is not None
        if do_cross_attention:
            print(f"shape in FlaxBlock: {receivers.shape}")
            cross_attention_outputs = self.layer[1](
                hidden_states,
                receivers=receivers,
                senders=senders,
...
class FlaxT5GraphPreTrainedModel(FlaxPreTrainedModel):
    ...
    def init_weights(self, rng: jax.random.PRNGKey, input_shape: Tuple, params: FrozenDict = None) -> FrozenDict:
        ...
        encdec_receivers = jnp.array([[jnp.arange(input_size)] * n_heads]*batch_size)
        # print(input_shape)
        # print(senders.shape)
